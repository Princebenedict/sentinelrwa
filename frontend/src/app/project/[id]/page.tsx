...
import { ArrowLeft, Brain, FileText, RefreshCw, Building2, Globe, Gauge } from "lucide-react";
...
export default function ProjectPage() {
  ...
  const barColor = health ? riskColors[health.risk_level as keyof typeof riskColors] || "#10b981" : "#10b981";
  const rated = health ? health.evaluation_count > 0 : false;
    ...
    <div className="min-h-screen">
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        ...
        {loading && !project ? (
          ...
          <>
            {project?.image_url && (
              <div className="w-full h-56 rounded-2xl overflow-hidden mb-6 border border-[#1e1e32] bg-[#0a0a12]">
            ...
            <div className="flex items-start justify-between mb-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="mt-2">
                    {health && <StarRating score={health.health_score} rated={rated} size="md" />}
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div className="glass-card p-5 md:col-span-2">
                  <div className="flex justify-between text-sm mb-3">
                    <span className="text-slate-400 flex items-center gap-1"><Gauge className="w-4 h-4" /> AI Validator Score</span>
                    <span className="font-mono font-bold text-xl" style={{ color: barColor }}>
                      {rated ? health.health_score + "/100" : "Not rated"}
                    </span>
                  </div>
                  <div className="health-bar mb-3">
                    <div className="health-bar-fill" style={{ width: (rated ? health.health_score : 0) + "%", backgroundColor: barColor }} />
                  </div>
                  <div className="grid grid-cols-3 gap-3 text-center text-xs">
                    <div className="bg-[#0a0a12] rounded-lg p-2">
                      <p className="text-slate-500">Risk</p>
                      <p className="text-white font-semibold mt-0.5">{health.risk_level}</p>
                    </div>
                    <div className="bg-[#0a0a12] rounded-lg p-2">
                      <p className="text-slate-500">Evaluations</p>
                      <p className="text-white font-semibold mt-0.5">{health.evaluation_count}</p>
                    </div>
                    <div className="bg-[#0a0a12] rounded-lg p-2">
                      <p className="text-slate-500">Evidence</p>
                      <p className="text-white font-semibold mt-0.5">{evidence.length}</p>
                    </div>
                  </div>
                </div>

                <div className="glass-card p-5">
                  <p className="text-slate-400 text-sm font-medium mb-3">Insurance</p>
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-slate-500">Review Trigger</span>
                      <span className="text-white font-mono">Below 40</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">Triggered</span>
                      <span className={insurance?.insurance_triggered ? "text-red-400" : "text-emerald-400"}>
                        {insurance?.insurance_triggered ? "Yes" : "No"}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">Claim Eligible</span>
                      <span className={insurance?.claim_eligible ? "text-red-400" : "text-slate-500"}>
                        {insurance?.claim_eligible ? "Yes" : "No"}
                      </span>
                    </div>
                  </div>
                </div>
            ...
            <div className="flex gap-1 bg-[#12121e] border border-[#1e1e32] rounded-xl p-1 mb-6 w-fit">
              {(["history", "evidence"] as const).map((tab) => (
                <button
                  ...
                  className={
                    activeTab === tab
                      ? "px-5 py-2 rounded-lg text-sm font-medium transition-all capitalize bg-indigo-600 text-white"
                      : "px-5 py-2 rounded-lg text-sm font-medium transition-all capitalize text-slate-500 hover:text-white"
                  }
                >
                  ...
                  [...evidence].reverse().map((ev, i) => (
                    <div key={i} className="glass-card p-4">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        {ev.source_url && (
                          <a href={ev.source_url} target="_blank" rel="noopener noreferrer" className="text-xs text-slate-500 hover:text-indigo-400 transition-colors truncate max-w-50">
                            {ev.source_url}
                          </a>
                        )}
                      </div>
